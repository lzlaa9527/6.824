...
// 如果目前还没有clerk等待该Op，且create为false直接返回；
// 如果create为true，说明存在clerk会监听该Op，创建并插入对应的OpResult，
// 等待clerk完成监听or[i].s时能够顺利被唤醒
func (or OpResults) SetAndBroadcast(i Index, op Op, re interface{}, create bool) {
	or.mu.Lock()
	defer or.mu.Unlock()
